...
async fn main() -> Result<()> {
            ...
            match event::read()? {
                Event::Key(key) => match key.code {
                    ...
                    KeyCode::Up => {
                        ...
                        table_state.select(Some(current.saturating_sub(3)));
                    }
                    KeyCode::Down => {
                        let current = table_state.selected().unwrap_or(0);
                        if let Some(tx_classes) = &cached_tx_classes {
                            let new = (current + 3).min(tx_classes.len().saturating_sub(1));
                            table_state.select(Some(new));
                        }
                    }
                    KeyCode::PageUp => {
                        let current = table_state.selected().unwrap_or(0);
                        table_state.select(Some(current.saturating_sub(15)));
                    }
                    KeyCode::PageDown => {
                        let current = table_state.selected().unwrap_or(0);
                        if let Some(tx_classes) = &cached_tx_classes {
                            let new = (current + 15).min(tx_classes.len().saturating_sub(1));
                            table_state.select(Some(new));
                        }
                    }
                    ...
                    KeyCode::End => {
                        if let Some(tx_classes) = &cached_tx_classes {
                            table_state.select(Some(tx_classes.len().saturating_sub(1)));
                        }
                    }
                    _ => {}
                },
                Event::Mouse(mouse) => {
                    match mouse.kind {
                        MouseEventKind::Down(_) => {
                            ...
                            let relative_y = (mouse.row as u16).saturating_sub(table_top_y);
                            if relative_y >= header_offset {
                                let row = (relative_y - header_offset) as usize;
                                if let Some(tx_classes) = &cached_tx_classes {
                                    if row < tx_classes.len() {
                                        ...
                                        println!(
                                            "\x1b[33mTriple-click or drag-select the line above to copy\x1b[0m\n"
                                        );
                                    }
                                }
                            }
                        ...
                        MouseEventKind::ScrollUp => {
                            let current = table_state.selected().unwrap_or(0);
                            if current > 0 {
                                if let Some(_) = &cached_tx_classes {
                                    table_state.select(Some(current - 1));
                                }
                            }
                        }
    ...
    let shielded_pct = 100.0 - transparent_pct;
    md.push_str(&format!("| Transparent | {:.1}% |\n", transparent_pct));
    md.push_str(&format!("| Total Shielded | {:.1}% |\n", shielded_pct));
    md.push_str("\n### Detailed Shielded Types\n");
    for (typ, cnt) in historical_counts
        .iter()
        .filter(|(t, _)| *t != "Transparent")
    {
...
async fn get_block_height(
    ...
    let payload =
        json!({ "jsonrpc": "2.0", "method": "getblockchaininfo", "params": [], "id": 99 });
